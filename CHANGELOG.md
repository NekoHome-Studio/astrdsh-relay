...
> 这是第三方复测报告（`astrdsh-relay_v0.9.0_复测报告_2026-09-26.md`）抓到的，
> 报告把根因钉得很准，这里照它的结论修，并把「为什么会这样」写下来。
> **`bridgeVersion` 仍是 `6`**，产品码行为零变化。

### 修复

- **三个 chain 测试在「插件已部署」的机器上全部变红**（本机不复现，因为本机插件目录里
  没有 `node_modules`）。根因是测试自己搭桩的方式：
  `ensureStubs()` 里那句 `if (existsSync(dir)) continue`——本意是「别覆盖真实的
  `@deepseek-ai` 包」（上一轮真的覆盖过一次，靠备份还原），但它把**一个响亮的 bug
  换成了一个安静的 bug**：
  1. 真实依赖在位 ⇒ 桩**从未生效**；
  2. 于是 `materializeDefaults(Config)` 面对真 Schema，它**没有** `__shape`；
  3. 物化出 `{}` ⇒ 插件加载期的 `sessionTitleTemplate` 强校验抛出；
  4. **该测试的全部断言同时变红**，报错长得像功能回归。
  结果是「CI 绿、本地红」，且红得极具误导性。

**修法（三件一起做，缺一条都还留着半个坑）：**

1. **隔离副本**（新增 `scripts/_fake-host.mjs`）：把插件的 `lib/` 与 `package.json`
   复制到 `.test-tmp/<label>/pkg/`，桩只建在那份副本里，再从副本 import。
   于是真实依赖在不在位**都不影响结论**，且真实目录**永远不被写入**——
   「桩覆盖真实包」这类事故从结构上不可能再发生。
2. **响亮失败**：`materializeDefaults` 拿不到 `__shape`（或一个默认值都物化不出）时
   **直接抛**，并说明「这通常意味着桩没生效」。环境问题就报成环境问题。
3. **共用一份**：原先三个 chain 测试各自复制了一份同样的桩逻辑（同一个 latent bug
   三份）。现在统一走 `_fake-host.mjs`。

### 新增

- `scripts/test-fake-host.mjs`（`npm run test:fake-host`，12 项）：**给这套假宿主本身
  写测试**——物化失败必须抛、隔离副本不碰真实目录、副本必须带 `package.json`
  （少了它 `.js` 会被当 CJS 解析，报错对不上真实原因）、以及一条**漂移门禁**：
  `lib/` 里 import 的每个 `@deepseek-ai/*` 要么有桩、要么在 `package.json` 的依赖里，
  否则报错提醒同步 `STUBS`（新增宿主机依赖时不会静默漏桩）。
  它排在 chain 测试**之前**跑：基础设施坏了，后面那些红是没有意义的噪音。

### 验证

- 修完在**两种环境**下结论一致：插件目录干净时 15/9/13；
  **在插件目录里放进 4 个「真实感」依赖（有 `.dict`、无 `__shape`）之后仍是 15/9/13**。
  后者正是复测报告里那 15 条全红的触发条件——修复前它必然全红。

### 文档

- `docs/DEPLOY-CHECKLIST.md` 的记录表加了说明：那是**首轮 alpha 部署的历史快照**，
  不是「当前应部署的版本」（复测报告正确地指出了这处可能被误读）。
- 复测报告余下待办的处理：
  - **`Schema.resolve(Config, {})` 抛 `unsupported type "undefined"`** ——
    已核实**产品码不使用它**（`lib/` 里的 `.resolve(` 全是 `Promise.resolve` 与
    `presets.resolve`，与 schemastery 无关）；用它的只有客户端包
    （`dsh-client-*`）。所以这条不影响本插件，也不需要跟。
  - `test:im-heartbeat` / `test:im-commands` 被 `&&` 短路遮住：短路正是因为上面那条
    缺陷；它修好之后这两步就会真正跑到（本机一直是绿的，需要 `aiohttp`）。
...
> **`bridgeVersion` 仍是 `6`**。
>
> **本版与 alpha 最大的区别是「有过真机」**：星驿完成了首次真实部署并跑通——两个群、
> 各自独立的 DSH 会话与标题、完整一轮对话（会话存档 18 条事件，含
> `turn/start` → `assistant/message` → `turn/end`）。过程中暴露的坑（DSH 侧从未安装、
> AstrBot 改配置后不重读、`--dump-config` 会把 token 打到终端）连同修法都写进了
> 部署清单 §12。
>
> **仍未实测的两项照旧标着未测**：契约 §18.8 的 B 半地基、A 半的长 turn 误报率
> —— 见部署清单 §11 记录表。**别把「接口通了」当成「能力验过了」。**

### 新增

- **`scripts/verify-bridge-live.py`**（`npm run verify:bridge`）：重启 DSH 之后的一键验收。
  从 profile 的 patch 里读 `token` / `pathPrefix` / `cwd`（**换机器不用改脚本**），
  检查：路由挂没挂、`bridgeVersion` 是不是 6、四条路由是否都返回 200、
  鉴权是否 fail-closed、`state.json` 是否已生成。**token 只打 sha256 前 12 位**。
- **`scripts/test-location-live.mjs`**（`npm run test:location-live`）：
  「定位不同会话集」的实测。读会话存档验四条链路——正向定位（§12.1）、
  映射集合（§12.2）、**反向定位（§12.4：把 `session/title` 事件与接口报的标题对比）**、
  不同会话集（不同 IM 对话是否落在不同会话）。
  **只有一个 IM 对话时第 4 条恒真，脚本会自己把这句打出来。**
- 两个脚本都**不进 `npm test`**：它们需要活的桥（网络 + token），
  混进 CI 只会变成「本地全绿、CI 必然失败」。

### 文档

- 部署清单新增 **§12「首次真机部署踩到的坑」**（五条，都带修法）与 **§4.1「定位不同会话集」**；
  **§11 记录表填上真机结果**：`PLAN-v0.8.5 §5.3 部署侧同步与定版` → ✅ 通过，
  其余三项如实标「未测」。启动期报错对照表补上 `unauthorized` 与 404 的真实成因。
- `README.md` 交付物地图补上这两个验证脚本。

### 变更

- `package.json` 新增 `verify:bridge` 与 `test:location-live`。
  ⚠️ 仓库里已有一个 `test:location`（`lib/location.js` 的**纯函数**测试），
  两者不冲突，但别混。

## v0.9.0-alpha — 2026-09-26（预发布，已被 v0.9.0 取代）
