...
const REQUIRED = [
  ...
  // 客户端半边：漏了它，Web 面板只会表现为「设置页里没有这个分区」——
  // 一个安静的白屏式失败，所以在这里钉死。
  [DSH_DIR, 'client/client.js'],
  [DSH_DIR, 'README.md'],
...
> 接口通了不等于能力验过了。
>
> 🆕 **v0.9.1 WebUI 面板（第一版）**：星驿现在有自己的 Web 界面，长在 **DSH Web 里**
> （设置 → 星驿）。一张对话映射表把「哪个 IM 对话 ↔ 哪条 DSH 会话 ↔ 标题 ↔
> 最终生效的工作目录」摆在一起，外加契约版本 / 运行时长 / **IM 是否在听** / 在途投递。
> 「IM 是否在听」正是自主心跳 \`online\` 闸门取的那个判据——它变红就等于
> 「此刻发起心跳会白烧一次调用且回复没人取」。
>
> * **只读**：改指/认领仍在 IM 里用 \`/dsh\` 指令完成（写操作要过权限门，那道门还没建）。
> * **不是认证**：默认关（\`panelEnabled: false\`），开启后也只允许回环访问；
>   同一台机器上的任何进程都能读这份快照。跨机请用反代加认证。
> * **不升版本**：面板路由不进契约常量表（IM 永远不会调它），
>   \`BRIDGE_VERSION\` **仍是 \`6\`**。
> * **需要重启 DSH** 才会出现：客户端半边随页面加载，光改配置不够。
> * ⚠️ **一处诚实标注**：客户端 \`inject\` 声明是照着本机已装的 \`dsh-skin-market\`
>   抄的，\`slots\` 服务由哪个包 provide **未静态确认**；装上后若设置页里没有「星驿」，
>   第一处要查的就是它（见 \`docs/WEBUI.md\` §6）。
>
> 🔧 **v0.9.2 修复（测试隔离缺陷，由第三方复测报告发现）**：三个 chain 测试在
> **「插件已部署」的机器上会全部变红**，而报错长得像功能回归。根因是测试搭桩的方式：
> \`ensureStubs()\` 里 \`if (existsSync(dir)) continue\` 本意是「别覆盖真实的
> \`@deepseek-ai\` 包」，代价却是让桩**静默失效** → \`materializeDefaults\` 面对真 Schema
> （它没有 \`__shape\`）→ 物化出 \`{}\` → 插件加载期的 \`sessionTitleTemplate\` 强校验抛出
> → **该测试全部断言同时变红**，看起来像 15 个功能回归。**CI 绿、本地红，误导性极强。**
>
> 修法三件一起做：① **隔离副本**（新增 \`scripts/_fake-host.mjs\`，把 \`lib/\` 复制到
> \`.test-tmp/\` 再在那里搭桩，**真实目录永不被写入**）；② **响亮失败**（物化拿不到形状
> 直接抛，环境问题就报成环境问题）；③ **共用一份**（原先三个测试各复制了一份同样的桩
> 逻辑，同一个 latent bug 三份）。另新增 \`scripts/test-fake-host.mjs\`（12 项）给这套
> 假宿主本身写测试，含一条 \`STUBS\` 漂移门禁。**已实测：把 4 个「真实感」依赖放进插件
