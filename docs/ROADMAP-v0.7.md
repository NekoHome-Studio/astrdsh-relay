...
>
> ⚠️ **2026-09-26 复核（v0.8.8 之后）**：本表是 v0.7 时代的快照，**多处已落地但正文未回改**。
> 逐条状态见 §1.1「v0.8.x 期间的进展」；**不要把下面的正文当现状读**。
> 保留原文是因为它是当时的判断依据（哪些算欠账、为什么），删掉就只剩结论、没有理由了。
...
### 1.1 v0.8.x 期间的进展（2026-09-26 复核）

| 原文条目 | 现状 | 落在哪 |
|---|---|---|
| §2.1 `user-questions` 转发「留二期」 | ✅ **已落地** | v0.8.6 实现、v0.8.8 补写契约 **§16**（问答双通道、`POST /answer`、逐题补交）。原文那句「留二期」已过期 |
| §2.5 P4 收尾：「`push_to_session` 仍固定纯文本」 | ✅ **已修** | v0.8.9：`reply_render_mode=card` 现在对**所有**出站消息生效（回复 / 心跳播报 / 主动消息），共用同一条「任一片失败就整条退回纯文本」的判定（`Main._card_urls`）。原文把它列为遗留是对的——配置键在一条路径上说了谎，正是 §1 要消灭的形状 |
| §2.3 P5 控制面整体未开 | ❌ **仍未开** | 仍卡在 §4 的 R4 决策（connector 能力是否并入） |
| §2.4 P3 收尾：DSH 重启恢复 / 卸载收尾顺序 | ⚠️ **部分** | 卸载收尾顺序已在 v0.8.7 修过一处真缺陷（`Main._health_task` 挂错类 → `terminate` 崩），见 CHANGELOG v0.8.7；**DSH 重启恢复仍未验证** |
| §3 未实测项 | 见下 | `throughSeq` 已结案（v0.8.2）、标题写入已落地（v0.7.1）；其余三条仍未核实，且都需要活宿主 |
| §5 建议切片的 `v0.7.x` 一行 | 🔁 **已过期** | 实际走的是 v0.8.x：fork/adopt/问答/心跳/主动消息/agent 文案。规划文档落后于实现是常态，但**别拿它排期** |
| §1 那两个空转配置键 | ✅ **已收口** | 见 §11（白名单并成一洞）+ v0.7.0 的 `health_interval_ms` / `reply_render_mode` |
...
> ⚠️ 本节是 v0.7 当时的快照。**第 1、5 条已经落地**（见 §1.1），读之前先看那张表。

1. **`user-questions` 转发**：契约 §5.3 / §9 #6 明确留二期。
   > ✅ **已过期**：v0.8.6 已实现，契约 **§16** 是它的正章。原文「留二期」不再成立。
...
5. **P4 收尾**：`card` 模式 —— **已落地**（§7），遗留仅「`push_to_session` 仍固定纯文本」。
   > ✅ **该遗留已修**（v0.8.9）：`reply_render_mode=card` 现在对回复、心跳播报、主动消息一视同仁。
...
| **不排期** | ~~契约显式不做项：`user-questions` 转发~~、v1 暴露流式控制面端点（必须走 `ctx.typertGateway.stream`） | 设计取舍，不是欠账。**注**：`user-questions` 已在 v0.8.6 落地（契约 §16），这一格只剩流式控制面 |
...
- `push_to_session` 仍固定纯文本：主动推送没有事件对象，
  ...
  > ✅ **v0.8.9 已做**，而且当初那句「未验证」正是关键：现在核实清楚了，
  > ``event.image_result`` 的分派规则就是「http → ``Image.fromURL``、
  > 否则 ``Image.fromFileSystem``」（`message_event_result.py:93-116`），
  > 所以主动推送照抄这条规则即可（`Main._image_chain`）。
  > 教训：**「未验证」是会过期的**——它是一张欠条，不是一句结论。
...
- `push_to_session` 仍固定纯文本（沿用 §7.4 的结论；**该条已于 v0.8.9 修**，见 §1.1）。
